...
description: "Create's filter system controls which items flow through funnels, tunnels, and arms. Learn basic filters, attribute filters, list filters, and tag-based matching."
---

Filters control item flow by matching items against a set of criteria. They appear everywhere in Create's logistics layer — in Brass Funnels, Smart Chutes, Brass Tunnels, Mechanical Arms, and Stock Keepers. Mastering the filter system is the key to building sorted, waste-free item networks that only move what you intend.
...
The Attribute Filter includes a built-in **InTagAttribute** entry that lets you match all items sharing a specific item tag — for example, `#minecraft:logs` to match any log type, or `#forge:ingots/iron` to match all iron ingot variants. This is far more maintainable than listing every item individually, especially in modded environments where multiple mods add variants of the same material.
...
## Stock Keeper and Threshold Switch
...
<CardGroup cols={2}>
  ...
  <Card title="Stock Keeper" icon="clipboard">
    Tracks an adjacent storage block and maintains a configured stock level for each item type. When stock of a configured item falls below its target quantity, the Stock Keeper automatically requests a replenishment package from linked Packagers. Uses the same filter items to define which items to track.
  </Card>
</CardGroup>

<Info>
  From version 6.0.10 onward, the Stock Keeper's Recipe Viewer integration supports four sync modes: **Disabled**, **From Viewer** (push requirements from JEI/REI into the keeper), **To Viewer** (show keeper contents in the viewer), and **Both**. This makes it easy to pull a recipe's ingredient list directly into a Stock Keeper's tracking configuration.
</Info>
