---
title: "Addon Development for Create: Getting Started"
sidebarTitle: "Overview"
description: "Build addons for the Create mod using NeoForge's event system and Create's Java API. Learn how to depend on Create, access its registries, and extend its core systems."
---
...
Add Create to your `build.gradle` dependencies block. Always use the variant that matches your target Minecraft and NeoForge versions. The `slim` classifier excludes runtime assets you don't need at compile time.

```groovy build.gradle
dependencies {
    // Create for NeoForge — use the version matching your target Minecraft/NeoForge
    implementation fg.deobf("com.simibubi.create:create-1.21.1:6.0.11:slim") { transitive = false }
    // Or via maven (createmod.net maven)
    // maven { url 'https://maven.createmod.net' }
}
```

<Note>
  The current stable release is **6.0.11** targeting Minecraft **1.21.1** with NeoForge **21.1.219**. Check [maven.createmod.net](https://maven.createmod.net) for the latest versions.
</Note>
